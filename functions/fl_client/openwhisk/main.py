...
class Client:
    def __init__(self, lr, optim, local_epochs,  model, server_weights, data_client,
        ...
        train_images = pickle.loads(requests.get(self.train_images_url, proxies=self.proxies, allow_redirects=True).content)
        train_labels = pickle.loads(requests.get(self.train_labels_url, proxies=self.proxies, allow_redirects=True).content)
    ...
    def create_datasetObject(self):
        ...
        print("Cardinality Client {0} is {1}".format(self.client_id,
                                                     tf.data.experimental.cardinality(self.dataset_train).numpy()))
...
def main(request):
    ...
    request_json = loads(request_json)
    request_json = bson.BSON(request_json).decode()
